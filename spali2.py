...
def find_spline_transform(a_t, sm, tm ):
 ...
 sbs_x = interpolate.SmoothBivariateSpline(template_coo[:, 0],
    template_coo[:, 1],(template_coo[:, 0]- source_coo[:, 0]), kx=kx, ky=ky)

 sbs_y = interpolate.SmoothBivariateSpline(template_coo[:, 0],
    template_coo[:, 1],(template_coo[:, 1]- source_coo[:, 1]), kx=kx, ky=ky)
...
#########################################################################################
def find_affine_transform(t_quad, s_quad,s_coo, t_coo, t_cat, s_cat, minmatchdist=5, minnmatch=200, maxcands=10, minquaddist=0.005):
 """
...
def run_sex(img_name):
 ...
 subprocess.call(['sex', '-c', './configfls/spali.sex', img_name, '-CATALOG_NAME', catname])
 return (catname)
#########################################################################################
...
def align(sci, a_t, sbs_x, sbs_y):
 ...
 source_fits.writeto('Aligned_sci.fits', overwrite=True)
#########################################################################################
...
def spalipy(sci, ref):
 """
 Credit to Joe Lyman. This is a mod version of his algorithm to 
 ...
 #at = affine_transform
 at, nmatch, source_matchdets, template_matchdets = find_affine_transform(r_quad_coo[0], s_quad_coo[0], s_quad_coo[1], r_quad_coo[1], r_cat, s_cat)

 sbs_x, sbs_y = find_spline_transform(at, source_matchdets, template_matchdets)
 
 align(sci, at, sbs_x, sbs_y)
...
if __name__ == '__main__':
 import time
 from ZOGYP import rdfits
 poi = rdfits(sys.argv[1])
 roi = rdfits(sys.argv[2])
 t0 = time.time()
 spalipy(poi, roi)
 t1 = time.time()
 print(t1-t0)
